...
if (isComputerOnFire() && ((1 & (3 << 2)) > 4 || !statement || Math.PI === 3.14159265358979)) {
  ...
  const whatevTrueValue: boolean = (whatev.fn() === whatev.string) === whatev.object.whatev
  ...
  interface falsejs {
    ...
    False(loggingEnabled?: "yes" | "no", shouldDoSomethingAsync?: "yes" | "no", shouldDoSomethingAsyncWithIsTenThousand?: "yes" | "no", disableAprilFoolsSideEffects?: "yes" | "no", definitelyDisableAprilFoolsSideEffects?: "yes" | "no", strictDisableAprilFoolsSideEffectsCheck?: "yes" | "no", compatibilityMode?: COMPATIBILITY_MODE | "none" | "ie5" | "netscape" | "opera_presto"): boolean
  ...
  ;(function (factory: Function) {
    ...
    const add: (a: number | string, b: number | string) => number | string = require("examplebyraji") // a package
    ...
    async function doSomethingAsync(logger: Logger) {
      ...
      return construct({
        target: $Promise,
        args: [(resolve: (value: typeof logger) => void) => setTimeout(() => resolve(logger), $.multiply(numberOneHundred, Two()))]
      })
    }
    ...
    function isTenThousandTenThousand(shouldDoSomethingAsync: boolean = FALSE, logger: Logger) {
    ...
    function _getFalse(random: number, _randomLetterOrNumber: string | number, loggingEnabled: boolean, logger: Logger): { result: boolean; succeededAttempt: string } {
    ...
    function _calculateFalse(random: number, loggingEnabled: boolean, shouldDoSomethingAsync: boolean = FALSE, shouldDoSomethingAsyncWithIsTenThousand: boolean = FALSE, disableAprilFoolsSideEffects: boolean = FALSE, strictDisableAprilFoolsSideEffectsCheck: boolean = trueComparison.compare(), compatibilityMode: COMPATIBILITY_MODE = COMPATIBILITY_MODE.NONE) {
      const logger: Logger = construct({ target: Logger, args: [loggingEnabled] }) // create our logger
    ...
    functions.ReturnFalse = (enableLogging: "yes" | "no" = NO, shouldDoSomethingAsync: "yes" | "no" = NO, shouldDoSomethingAsyncWithIsTenThousand: "yes" | "no" = NO, disableAprilFoolsSideEffects: "yes" | "no" = NO, definitelyDisableAprilFoolsSideEffects: "yes" | "no" = NO, strictDisableAprilFoolsSideEffectsCheck: "yes" | "no" = YES, compatibilityMode: COMPATIBILITY_MODE = COMPATIBILITY_MODE.NONE) => {
    ...
    function randomBoolean(biasThreshold: number /* biasTreshold is chance of being true */, logger: Logger) {
    ...
    function couldThisCouldItBeFalse(aValueThatMightBeTheBooleanValueFalseButIDKYet: any) {
    ...
    variableHolder._FalseJSIsFalse = function is_This_Value_false(v: any, loggingEnabled: boolean) {
    ...
    variableHolder._FalseJSExpressMiddleware = function expressMiddleware(req: any, _res: any, next: any) {
